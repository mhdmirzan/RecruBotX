...
class VoiceAgent:
    ...
    def __init__(self, interview_field=None, num_questions=None, position_level=None):
        ...
        if not deepgram_key:
            print("\n⚠️ DEEPGRAM_API_KEY not found in environment variables.")
    ...
    def record_audio(self, max_duration=60, sample_rate=16000, silence_duration=4.0):
        ...
        print(f"🎤 Recording... (up to {max_duration}s, stops after {silence_duration}s silence)")
        chunk_ms = 500
    ...
    def speech_to_text(self, max_duration=60, sample_rate=16000, silence_duration=4.0):
        ...
        print("🔄 Streaming speech to text with Deepgram Nova-2...")
        chunk_ms = 250
        chunk_samples = int(sample_rate * chunk_ms / 1000)
        max_samples = int(max_duration * sample_rate)
        silence_ms = 0
...
Return ONLY the short question text, ending with a question mark. No preamble, no explanations, no numbering."""
            ...
            try:
                ...
                if question and len(question.split()) >= 2:
                    ...
                    if self._is_duplicate_question(question):
                        print(f"⚠️ Duplicate detected: '{question}' - Regenerating...")
                        continue  # Try again
                    
                    # Question is unique, track it
                    self._extract_and_track_topic(question)
                    print(f"✅ Question {self.question_count}/{self.max_questions} generated: {question}")
                    return question
                else:
        ...
        print("⚠️ Max retries reached, using fallback question")
    ...
    def _extract_and_track_topic(self, question):
        ...
        if tech_terms:
            ...
            print(f"🔖 Tracked topic: {topic_signature}")
    ...
    def analyze_answer(self, question, user_answer):
        ...
        print("🔍 Analyzing answer with deep AI evaluation...")
...
❌ WRONG CONCEPT = max 39/100 (even if long answer)
❌ VAGUE/UNCERTAIN ("I think", "maybe") = max 55/100
❌ MEMORIZED WITHOUT UNDERSTANDING = max 65/100
❌ CIRCULAR DEFINITIONS or BUZZWORDS WITHOUT EXPLANATION = max 50/100
❌ CONFUSING SIMILAR CONCEPTS = max 45/100

✅ CORRECT + BRIEF = 60-75/100
✅ UNDERSTOOD + COMPLETE = 76-85/100
✅ MASTERY DEMONSTRATED = 86-100/100
...
Example: "Score: 82/100 - Demonstrates strong conceptual understanding with accurate explanation and practical context, could add more depth on edge cases.\""""
    ...
    def generate_report(self):
        ...
        print("\n" + "="*60)
        print("📊 SOFTWARE ENGINEERING INTERVIEW REPORT")
        print("="*60 + "\n")
        ...
        print(f"📋 Interview Type: {self.interview_type}")
        print(f"💼 Position Level: {self.position_level}")
        print(f"📝 Total Questions: {len(self.interview_data['questions'])}")
        print(f"⭐ Overall Score: {avg_score:.1f}/100")
        print(f"📈 Performance Level: {self._get_performance_level(avg_score)}\n")
    ...
    def _get_performance_level(self, avg_score):
        ...
        if avg_score >= 85:
            return "🌟 Excellent - Strong Candidate"
        elif avg_score >= 70:
            return "✅ Good - Recommended"
        elif avg_score >= 55:
            return "⚠️ Fair - Needs Improvement"
        elif avg_score >= 40:
            return "❌ Below Standard - Not Recommended"
        else:
            return "❌❌ Failing - Unacceptable Performance"
    ...
    def _save_report_to_file(self, avg_score):
        ...
        try:
            ...
            print(f"✅ Report saved to: {txt_filename} and {pdf_filename}")
            return pdf_filename
        except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            print(f"⚠️ Could not speak summary: {e}")
