...
const CandidateReport = () => {
    ...
    const handleDownloadReport = async () => {
        if (!report) return;
        try {
            const response = await fetch(`${API_BASE_URL}/rankings/evaluations/${rankingId}/download`);
    ...
    const formatListElements = (textStr) => {
        if (!textStr) return [];
        if (Array.isArray(textStr)) return textStr;
        return textStr.split('\n')
            .map(s => s.replace(/^[-*]\s*/, '').trim())
            .filter(s => s.length > 0 && !s.toLowerCase().startsWith("strengths") && !s.toLowerCase().startsWith("weaknesses"));
    };
                            ...
                            <div className="col-span-2 space-y-6">
                                ...
                                <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
                                    ...
                                    <div className="prose prose-blue max-w-none text-gray-600 leading-relaxed font-medium">
                                        {report.summary || report.detailedAnalysis || "Analysis securely generated by AI based on candidate's combined technical and communication capabilities."}
                                    </div>
