...
const DoubleColumnBuilder = ({ user, handleLogout, showPreview, setShowPreview }) => {
    const [resume, setResume] = useState({
        ...
        certifications: [""],
        languages: [""], // Unique to Double Column template
        references: "",
    });
    ...
    if (showPreview) {
        return (
            <div className="h-full overflow-y-auto bg-white rounded-2xl shadow-lg p-8">
                <ResumePreview resume={safeResume} template="double-column" />
            </div>
        );
    }
